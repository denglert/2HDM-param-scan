...
//  double mA = sqrt(mH_in*mH_in*(1-cba_in*cba_in) + mh_in*mh_in*cba_in*cba_in-Z5_in*Hvev_2);
  ...
  if ( BitWriteLHA )
  {
	std::stringstream filename_LHA;
	filename_LHA << "./LHA/mh_";
	filename_LHA << std::setprecision(4) << std::fixed << mh_in;
	filename_LHA << "mH_";
	filename_LHA << std::setprecision(4) << std::fixed << mH_in;
	filename_LHA << "cba_";
	filename_LHA << std::setprecision(4) << std::fixed << cba_in;
	filename_LHA << "tb_";
	filename_LHA << std::setprecision(4) << std::fixed << tanb_in;
	filename_LHA << "Z4_";
	filename_LHA << std::setprecision(4) << std::fixed << Z4_in;
	filename_LHA << "Z5_";
	filename_LHA << std::setprecision(4) << std::fixed << Z5_in;
	filename_LHA << "Z7_";
	filename_LHA << std::setprecision(4) << std::fixed << Z7_in;
	filename_LHA << ".lha";

  //std::string filename_LHA = "./LHA/mh_%.4f_mH_%.4f_cba_%.4f_tb_%.4f_Z4_%.4f_Z5_%.4f_Z7_%.4f.lha", mh_in, mH_in, cba_in, tanb_in, Z4_in, Z5_in, Z7_in);
  model.write_LesHouches(filename_LHA.str().c_str(), 1, 0, 1, 1);
  }
  ...
  std::string filename_param_chisq = "./chisquare_table.dat";

  std::ofstream file_param_chisq;
  file_param_chisq.open(filename_param_chisq.c_str(), std::ios_base::app);

  std::stringstream line;
  line << std::setprecision(6) << std::fixed << mh_in << " ";
  line << std::setprecision(6) << std::fixed << mH_in << " ";
  line << std::setprecision(6) << std::fixed << cba_in << " ";
  line << std::setprecision(6) << std::fixed << tanb_in << " ";
  line << std::setprecision(6) << std::fixed << Z4_in << " ";
  line << std::setprecision(6) << std::fixed << Z5_in << " ";
  line << std::setprecision(6) << std::fixed << Z7_in << " ";
  line << std::setprecision(6) << std::fixed << csqtot << " ";
  line << std::setprecision(6) << std::fixed << tot_hbobs << " ";
  line << std::setprecision(1) << std::fixed << BitAllowedStability << " ";
  line << std::setprecision(1) << std::fixed << BitAllowedUnitarity << " ";
  line << std::setprecision(1) << std::fixed << BitAllowedPerturbativity << " ";
  line << std::setprecision(6) << std::fixed << mA << " ";
  line << std::setprecision(6) << std::fixed << Gamma_h << " ";
  line << std::setprecision(6) << std::fixed << Gamma_A << " ";
  line << std::setprecision(6) << std::fixed << mHp << " ";
  line << std::setprecision(6) << std::fixed << l6 << " ";
  line << std::setprecision(6) << std::fixed << l7 << " ";
  line << std::setprecision(6) << std::fixed << m12_2 << " ";
  line << std::setprecision(6) << std::fixed << S << " ";
  line << std::setprecision(6) << std::fixed << T << " ";
  line << std::setprecision(6) << std::fixed << U << " ";
  line << std::setprecision(6) << std::fixed << V << " ";
  line << std::setprecision(6) << std::fixed << W << " ";
  line << std::setprecision(6) << std::fixed << X << "\n";
//  std::string line = Form("%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6e %d %d %d %.6f %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e\n", mh_in, mH_in, cba_in, tanb_in, Z4_in, Z5_in, Z7_in, csqtot, tot_hbobs, BitAllowedStability, BitAllowedUnitarity, BitAllowedPerturbativity, mA, Gamma_h, Gamma_A, mHp, l6, l7, m12_2, S, T, U, V, W, X );
